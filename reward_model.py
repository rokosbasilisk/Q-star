...
class RewardModelTrainer(object):
    ...
    ################################################################################################################    
    def get_true_rewards(self, batch): 
        ## TODO: you need to modify this 
        return true_rewards

    def get_loss(self, batch):

        # get the corresponding true rewards
        true_rewards = self.get_true_rewards(batch) # a list of rewards.
    ...
    def train(self):
        ...
        for epoch in range(ceil(self.rew_num_train_epochs)):
            ...
            for step, batch in enumerate(self.train_dataloader): # self.train_dataloader
                batch = {k: v.to(self.device) for k, v in batch.items()}
